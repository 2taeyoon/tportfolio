...
export function useDashAnimation({ delay = 2000, duration = 2000, intervalTime = 20, startValue = 25 }: UseDashAnimationProps) {
			...
			const timeout = setTimeout(() => {
				...
				interval = setInterval(() => {
						...
						if (currentDash <= 1) {
								...
								setDashArray(0); // 마지막 값은 0으로 고정
						} else {
								setDashArray(currentDash); // <=1 될 때까지 실행
						}
    ...
    }, [delay, duration, intervalTime, startValue]); // 해당 값이 변경되면 애니메이션 재실행
